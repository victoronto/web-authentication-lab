...
Here I have to change the localhost in `redis.Redis(host='localhost', port=6379, db=0)` because the redis container is running in DooD. Before host.docker.internal works well, but it's no longer works.

host.docker.internal 在你的环境里now解析到了一个错误的 IP（0.250.250.254），现在不能用了。Gateway IP 是你刚验证过能连上 Redis 的，所以现在用它能解决问题。
